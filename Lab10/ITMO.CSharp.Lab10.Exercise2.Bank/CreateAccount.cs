...
namespace ITMO.CSharp.Lab10.Exercise2.Bank
{
    class CreateAccount
    {
        static void Main()
        {
            ...
            long sidsAccNo = BankMethods.CreateAccount();
            BankAccount sids = BankMethods.GetAccount(sidsAccNo);
            TestDeposit(sids);
            TestWithdraw(sids);
            Write(sids);
            if (BankMethods.CloseAccount(sidsAccNo))
